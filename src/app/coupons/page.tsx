    ...
    return (
      <div className="flex flex-wrap gap-1">
        {codeArray.map((code, idx) => (
          <span key={idx} className="inline-block bg-gray-100 text-gray-700 px-2 py-0.5 rounded text-xs font-mono">
            {code}
          </span>
        ))}
      </div>
    ...
    <div className="p-4 md:p-8 bg-gray-100 min-h-[calc(100vh-4rem)] lg:min-h-screen">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 md:p-6 mb-6 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-gray-900">کوپن‌های خریداری شده</h1>
            <p className="text-sm text-gray-500 mt-1">لیست تمامی ووچرها و کوپن‌های صادر شده</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center p-12 text-center">
              ...
              <p className="text-lg font-medium text-gray-500">هیچ کوپنی یافت نشد</p>
            ...
            <>
              ...
              <div className="md:hidden divide-y divide-gray-200">
                {vouchers.map((voucher) => (
                  <div key={`${voucher.order_id}-${voucher.order_item_id}`} className="p-4 hover:bg-gray-50 transition-colors">
                    ...
                    <div className="mb-3">
                      <span className="text-xs text-gray-500 block mb-1">کد کوپن:</span>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs text-gray-500 mb-3 bg-gray-50 p-3 rounded-lg">
                      ...
                      <div>وضعیت سفارش: <span className="font-medium text-gray-700">{renderOrderStatus(voucher.order_status).text}</span></div>
                      <div>قیمت: <span className="font-medium text-gray-700">{formatPrice(voucher.voucher_price)}</span></div>
                    </div>
                    
                    <div className="flex justify-between items-center pt-3 border-t border-gray-100">
                      <span className="text-xs text-gray-500">تاریخ استفاده: {formatDate(voucher.redeemed_at)}</span>
                      <span className="text-xs text-gray-500">{formatDate(voucher.order_date)}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-right min-w-[1000px]">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {vouchers.map((voucher) => (
                      <tr key={`${voucher.order_id}-${voucher.order_item_id}`} className="hover:bg-gray-50 transition-colors align-top">
